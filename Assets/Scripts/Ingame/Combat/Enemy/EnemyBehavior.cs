...
namespace DeckRoguelike.Combat
{
    ...
    ///   2. EnemyBehaviorRegistry.Register("enemy_slime", () => new SlimeBehavior()) 로 등록합니다.
    ///   3. EnemyData.behaviorId 에 같은 ID 문자열을 설정합니다.
    ///
    /// [인스턴스 방식]
    ///   행동 객체는 유닛 소환 시 유닛마다 개별 생성됩니다 (EnemyInstance.Behavior).
    ///   따라서 페이즈, 패턴 카운터 등 내부 상태를 멤버 변수로 안전하게 관리할 수 있습니다.
    /// </summary>
    public abstract class EnemyBehavior
    {
        ...
        public virtual void OnSpawn(EnemyInstance self, CombatController combat) { }

        /// <summary>
        /// 플레이어 턴 시작 시 호출됩니다. 이번 적 턴에 할 행동을 미리 결정하고 내부에 저장하세요.
        /// 공격 좌표는 plannedAttackPositions에 저장하면 자동으로 프리뷰에 반영됩니다.
        /// </summary>
        public virtual void PlanTurn(EnemyInstance self, CombatController combat) { }

        /// <summary>이 유닛의 턴마다 호출됩니다. PlanTurn에서 결정한 행동을 실행합니다.</summary>
        public abstract void ExecuteTurn(EnemyInstance self, CombatController combat);

        /// <summary>유닛이 사망할 때 호출됩니다.</summary>
        public virtual void OnDeath(EnemyInstance self, CombatController combat) { }

        /// <summary>UI에 표시할 다음 행동 설명 텍스트를 반환합니다.</summary>
        public virtual string GetIntentText(EnemyInstance self) => "공격";
